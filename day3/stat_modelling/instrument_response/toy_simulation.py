...
from .power_law import BoundedPowerLaw, BrokenBoundedPowerLaw
...
class ToySimulation(object):
    ...
    def __init__(
        ...
        @param systematic offset of detected energy in TeV
        @param Edep_systematic offset as fraction of mean detected energy
        """
        ...
        self.effective_area = EffectiveArea(
            effective_area_scale, effective_area_index, effective_area_break
        )
        ...
        self.calorimeter = Calorimeter(
            calorimeter_scale,
            calorimeter_break,
            calorimeter_uncertainty,
            systematic,
            Edep_systematic,
        )

    def run(self, N, T=1, seed=0, systematic=0, Edep_systematic=0):
        """
        Run the simulation
        ...
        # Effective area
        interaction_probability = self.effective_area.interaction_probability(
            self.true_energy
        )
        ...
        # Event generation
        # Each particle produces a number of secondary particles proportional to its energy.
        # The number of secondary particle follows a poisson distribution.
        expected_number_of_secondaries = (
            self.calorimeter.expected_number_of_secondaries(self.true_energy)
        )
        ...
        self.detected_fraction = np.random.normal(
            0.5, 0.1, len(self.number_of_secondaries)
        )
        self.number_of_detected_secondaries = (
            self.detected_fraction * self.number_of_secondaries
        )
        ...
        self.detected_energy = self.calorimeter.detected_energy(
            self.number_of_detected_secondaries, energy_per_secondary
        )
    ...
    def save(self, filename):
        ...
        with h5py.File(filename, "w") as f:
            ...
            f.create_dataset(
                "number_of_detected_secondaries",
                data=self.number_of_detected_secondaries,
            )
            ...
            try:
                ...
                f.create_dataset("power_law_index", data=-self.power_law.gamma1)
