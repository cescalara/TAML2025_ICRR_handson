...
class EffectiveArea(object):
    ...
    def __init__(self, scale, index, energy_break):
        ...
        E = np.linspace(1.0, energy_break * 10, int(1e4))
...
class Calorimeter(object):
    ...
    def __init__(
        ...
        self.Edep_systematic = Edep_systematic
    ...
    def detected_energy(self, number_of_detected_secondaries, energy_per_secondary):
        ...
        # Systematic effects
        detected_energy = (
            detected_energy + self.systematic + (self.Edep_systematic * mean)
        )
...
class Response(object):
    ...
    def __init__(
        ...
        detected_energy,
        effective_area_max,
        nbins_true_energy=50,
        nbins_detected_energy=50,
        min_energy=1,
        max_energy=100,
        detected_energy_bins=None,
    ):
        """
        Detector response information.
        ...
        @param detected_energy_bins fix detected energy bins, otherwise create
        """

        self.nbins_true_energy = nbins_true_energy
        self.nbins_detected_energy = nbins_detected_energy

        self.true_energy_bins = np.logspace(
            np.log(min_energy), np.log(max_energy), nbins_true_energy + 1, base=np.e
        )

        if type(detected_energy_bins) == np.ndarray:
            self.detected_energy_bins = detected_energy_bins
            self.nbins_detected_energy = len(detected_energy_bins) - 1
        else:
            self.detected_energy_bins = np.logspace(
                np.log(min(detected_energy)),
                np.log(max(detected_energy)),
                nbins_detected_energy + 1,
                base=np.e,
            )

        # Get energy dispersion histogram
        self.dN_dt_joint, _, _ = np.histogram2d(
            true_energy,
            detected_energy,
        ...
        self.matrix = np.zeros((self.nbins_true_energy, self.nbins_detected_energy))
        self.p_detected_given_true = np.zeros(
            (self.nbins_true_energy, self.nbins_detected_energy)
        )

        for i in range(self.nbins_true_energy):
            for j in range(self.nbins_detected_energy):
                self.matrix[i][j] = (
                    self.dN_dt_joint[i][j] / dN_dt_true[i]
                ) * effective_area[
                    i
                ]  # m^2
                self.p_detected_given_true[i][j] = (
                    self.dN_dt_joint[i][j] / dN_dt_true[i]
                )  # dimensionless
        ...
        p_smooth = gaussian_filter(
            self.p_detected_given_true, smoothing_standard_deviation
        )
        # Deal with -inf
        ...
        self._spline = RectBivariateSpline(
            self.true_centres, self.detected_centres, log_p_smooth, s=0.0
        )
    ...
    def get_log_conditionals(self, detected_energy, true_energy_grid):
        ...
        for Edet in detected_energy:
            ...
            normalised_conditional_log_probability.append(
                np.log(cond_prob / sum(cond_prob))
            )
    ...
    def show(self):
        ...
        norm = matplotlib.colors.Normalize(
            vmin=min(np.log(self.true_energy_bins[:-1])),
        ...
        for i, y in enumerate(y_values):
            ...
            ax.fill_between(
                ...
                y_add + y[idx],
                zorder=-10,
                color=cmap(norm(np.log(self.true_energy_bins[i]))),
                alpha=0.5,
            )
            y_add += delta_y
